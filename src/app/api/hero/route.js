...
const heroSchema = z.object({
  avatar: z
    ...
    .min(1)
    .refine(
      (v) => v.startsWith("data:") || v.startsWith("/") || v.startsWith("http"),
      "Avatar must be a data URL or image URL/path"
    ),
  full_name: z.string().trim().min(2).max(200),
  short_description: z.string().trim().min(2).max(120),
  long_description: z.string().trim().min(10).max(5000),
});
